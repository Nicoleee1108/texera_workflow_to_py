...
class WorkflowToPythonTranslator extends LazyLogging {
  ...
  def translate(
    ...
    // file would be overwritten by the next chart naming the same one, and a
    // workflow ending in two charts would keep a single picture.
  ...
  // can tell whose picture is whose, and numbered so that a plan holding two of
  // the same chart still writes two files. Numbered from one even when the plan
  // holds a single chart, so the name a script writes does not depend on what
  // else the plan happens to contain.
  ...
  // inside in10df). Only the code parts of the block are rewritten: a generator
  // writes a column name as a string literal, and a column may be named
  // `in1df`, where a blind replacement would ask the frame for a column called
  // `df1`. After substitution, scans the code for any leftover placeholders and
  // logs a warning — that signals a mismatch between an operator's declared
  // port count and what its generateStandaloneCode actually emits.
  private def substituteVars(
    ...
    // The scan reads the code segments joined by a newline rather than the
    // finished block, so that the text either side of a skipped literal cannot
    // spell a placeholder no operator wrote.
